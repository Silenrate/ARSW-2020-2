...
public class Snake extends Observable implements Runnable {
    ...
    public Snake(int idt, Cell head, int direction, AtomicInteger worstSnake, AtomicInteger maximumSnakeSize, AtomicInteger snakesAlive, Notifier notifier) {
        this.idt = idt;
        this.direction = direction;
        generateSnake(head);
        isPaused = false;
        this.worstSnake = worstSnake;
        this.maximumSnakeSize = maximumSnakeSize;
        this.snakesAlive = snakesAlive;
    ...
    public void run() {
        ...
        fixDirection(head);
        if (snakesAlive.decrementAndGet() == 0) {
            notifier.notifyEndOfGame();
        }
    }
    ...
    private void snakeCalc() {
        ...
        if (maximumSnakeSize.get() < snakeBody.size()) {
            maximumSnakeSize.set(snakeBody.size());
        }
    }
