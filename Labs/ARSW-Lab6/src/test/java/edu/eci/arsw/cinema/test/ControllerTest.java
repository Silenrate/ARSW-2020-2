...
public class ControllerTest {
    ...
    @Test
    public void shouldNotDeleteABadFunction() throws Exception {
        CinemaFunction cinemaFunction = services.getFunctionByMovieName("cinemaX","2018-12-18 15:30","SuperHeroesMovie");
        mvc.perform(
                MockMvcRequestBuilders.delete("/cinemas/cinemaX")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(gson.toJson(cinemaFunction.getMovie()))
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());
    ...
    @Test
    public void shouldNotDeleteAFunctionOfANonExistentCinema() throws Exception {
        mvc.perform(
                MockMvcRequestBuilders.delete("/cinemas/cineplus")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(gson.toJson(getCinemaFunction("Toy Story", "Infantil")))
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound());
    ...
    @Test
    public void shouldDeleteAFunction() throws Exception {
        CinemaFunction cinemaFunction = services.getFunctionByMovieName("cinemaX","2018-12-18 15:30","TheNight");
        List<CinemaFunction> cinemaFunctions = services.getFunctionsbyCinemaAndDate("cinemaX","2018-12-18");
        mvc.perform(
                MockMvcRequestBuilders.delete("/cinemas/cinemaX")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(gson.toJson(cinemaFunction))
        ...
        List<CinemaFunction> actualCinemaFunctions = services.getFunctionsbyCinemaAndDate("cinemaX","2018-12-18");
        assertEquals(cinemaFunctions.size()-1, actualCinemaFunctions.size());
        boolean found = false;
        for (CinemaFunction c: actualCinemaFunctions) {
            if(c.equals(cinemaFunction)){
                found = true;
                break;
            }
        }
